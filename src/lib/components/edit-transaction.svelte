    ...
    <Dialog.Content>
        ...
        <div class="grid gap-4 py-4">
                ...
                <Popover.Root openFocus>
                    <Popover.Trigger asChild let:builder>
                        <Button
                            ...
                            {date ? dateFormatter.format(date.toDate(getLocalTimeZone())) : "Select a date"}
                        </Button>
                    </Popover.Trigger>
                    <Popover.Content class="w-auto p-0">
                        <Calendar value={date} onValueChange={v => v && (date = v)} initialFocus />
                    </Popover.Content>
                </Popover.Root>
                </div>
            </div>
            <div class="flex flex-col items-start gap-1.5">
                ...
                <Input id="description" type="text" value={description} on:input={handleDescChange} />
            </div>
            <div class="grid gap-4">
                <div class="flex flex-col col-span-2 items-start gap-1.5 max-w-48">
                    <Label for="amount" class="text-right">Amount</Label>
                    <Input id="amount" type="number" value={amount} on:input={handleAmountChange} step="0.01" />
                </div>
                {#if txn.txnType === TransactionType.Debit}
                    <div class="flex flex-col items-start col-span-2 gap-1.5 max-w-48">
                        <Label for="category" class="text-right">Expense Category</Label>
                        <Select.Root selected={selectedExpenseCategory} onSelectedChange={v => v && (selectedExpenseCategory = v)}>
                            <Select.Trigger>
                                <Select.Value placeholder="Select a category">something</Select.Value>
                            </Select.Trigger>
                            <Select.Content>
                                {#each Object.entries(ExpenseCategory) as [key, value]}
                                    <Select.Item value={value}>
                                        <div class="flex items-center gap-2">
                                            <div class="text-xl">{ExpenseCategoryIcons[value]}</div>
                                            <div>{key}</div>
                                        </div>
                                    </Select.Item>
                                {/each}
        ...
        <Dialog.Footer>
            ...
            <Button size="sm" on:click={handleUpdate}>Update</Button>
        </Dialog.Footer>
    </Dialog.Content>
</Dialog.Root> -->
